...
/**
 * This class controls how Universities object interact with the system
 * 
...
public class UniversityController {
	...
	public ArrayList<University> searchSchools(String schoolName, String state, String location, String control,
			int nStuLow, int nStuHigh, int prctfLow, int prctfHigh, int svLow, int svHigh, int smLow, int smHigh,
			int eLow, int eHigh, int prctfinLow, int prctfinHigh, int nApLow, int nApHigh, int prctaLow, int prctaHigh,
			int prcteLow, int prcteHigh, int asLow, int asHigh, int ssLow, int ssHigh, int qLow, int qHigh,
		...
		searchList = allUnivs;
		results = searchByEmphasis(
				searchByQualityOfLife(
						searchBySocialLife(
								searchByAcademicScale(
										searchByPercEnrolled(
												searchByPercAdmitted(
														searchByNumOfApps(
																searchByPercentFinAid(searchByExpenses(searchBySATMath(
																		searchBySATVerbal(searchByPercentFemale(
																				searchByNumOfStudents(searchByControl(
																						searchByLocation(searchByState(
																								searchBySchool(
																										searchList,
																										schoolName),
																								state), location),
																						control), nStuLow, nStuHigh),
																				prctfLow, prctfHigh), svLow, svHigh),
																		smLow, smHigh), eLow, eHigh), prctfinLow,
																		prctfinHigh),
																nApLow, nApHigh),
														prctaLow, prctaHigh),
												prcteLow, prcteHigh),
	...
	/**
	 * Searches based on the location
	 * 
	 * @param temp
	 * @param l
	 * @return ArrayList of all universities matching search criteria
	 ...
	 * @return ArrayList of all universities matching search criteria
	 */
	public ArrayList<University> searchByEmphasis(ArrayList<University> temp, ArrayList<String> emp) {
		ArrayList<University> matchedSchools = new ArrayList<University>();
		...
		if (emp.get(0).equals("") && emp.get(1).equals("") && emp.get(2).equals("") && emp.get(3).equals("")
				&& emp.get(4).equals("")) {
			...
			for (University i : temp) {
				...
				for (String u : i.getEmp()) {
					if (u.equals(v.get(0)) || u.equals(v.get(1)) || u.equals(v.get(2)) || u.equals(v.get(3))
							|| u.equals(v.get(4))) {
						matchedSchools.add(i);
					}
				}
		...
		if (univ != null && inDatabase) {
			...
			for (University i : allUnivs) {
				...
				if (univ.getNumOfStudents() == -1) {
					...
					count += 2;
				} else if (i.getNumOfStudents() - univ.getNumOfStudents() <= 5000
						&& i.getNumOfStudents() - univ.getNumOfStudents() >= -5000) {
					count++;
				}
				...
				if (univ.getSatVerbal() == -1) {
					...
					count += 2;
				} else if (i.getSatVerbal() - univ.getSatVerbal() <= 25
						&& i.getSatVerbal() - univ.getSatVerbal() >= -25) {
					count++;
				}
				...
				if (univ.getExpenses() == -1) {
					...
					count += 2;
				} else if (i.getExpenses() - univ.getExpenses() <= 2500
						&& i.getExpenses() - univ.getExpenses() >= -2500) {
					count++;
				}
				...
				if (univ.getFinancialAid() == -1) {
					...
					count += 2;
				} else if (i.getFinancialAid() - univ.getFinancialAid() <= 5
						&& i.getFinancialAid() - univ.getFinancialAid() >= -5) {
					count++;
				}
				...
				if (univ.getNumOfApps() == -1) {
					...
					count += 2;
				} else if (i.getNumOfApps() - univ.getNumOfApps() <= 500
						&& i.getNumOfApps() - univ.getNumOfApps() >= -500) {
					count++;
				}
				...
				if (univ.getPerAdmitted() == -1) {
					...
					count += 2;
				} else if (i.getPerAdmitted() - univ.getPerAdmitted() <= 5
						&& i.getPerAdmitted() - univ.getPerAdmitted() >= -5) {
					count++;
				}
				...
				if (univ.getPerEnrolled() == -1) {
					...
					count += 2;
				} else if (i.getPerEnrolled() - univ.getPerEnrolled() <= 5
						&& i.getPerEnrolled() - univ.getPerEnrolled() >= -5) {
					count++;
				}
				...
				if (univ.getAcademicScale() == -1) {
					...
					count += 2;
				} else if (i.getAcademicScale() - univ.getAcademicScale() <= 1
						&& i.getAcademicScale() - univ.getAcademicScale() >= -1) {
					count++;
				}
	...
	public ArrayList<University> topRecommended2(University univ) {
		...
		if (univ != null && inDatabase) {
			...
			for (University i : allUnivs) {
				...
				if (i.getAcademicScale() != -1) {
					distance += Math.abs(univ.getAcademicScale() - i.getAcademicScale())
							/ (maxAcademics - minAcademics);
				}
				if (i.getSocialScale() != -1) {
	 ...
	 */
	public void editSchool(String school, String state, String location, String control, int numStudents, int perFem,
		...
		if (state.equals("ALABAMA") || state.equals("ALASKA") || state.equals("ARIZONA") || state.equals("ARKANSAS")
			...
			if (location.equals("-1") || location.equals("SMALL-CITY") || location.equals("SUBURBAN")
					|| location.equals("URBAN")) {
											...
											if (numOfApps == -1 || numOfApps >= 0) {
												if (0 <= perAdmitted && perAdmitted <= 100 || perAdmitted == -1) {
													if (0 <= perEnrolled && perEnrolled <= 100 || perEnrolled == -1) {
														if (0 <= academicScale && academicScale <= 5
																|| academicScale == -1) {
															if (0 <= socialScale && socialScale <= 5
																	|| socialScale == -1) {
																if (0 <= qualOfLife && qualOfLife <= 5
																		|| qualOfLife == -1) {
																	University newUniv = new University(school, state,
																			location, control, numStudents, perFem,
																			satVerbal, satMath, expenses, financialAid,
																			numOfApps, perAdmitted, perEnrolled,
	...
	/**
	 * uses toprecommend and toprecommend2 to make a more accurate recommendation
	 * system
	 * 
	 * @param univ
